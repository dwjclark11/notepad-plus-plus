...
// Linux/Qt port of FunctionCallTip.cpp
// This file replaces FunctionCallTip.cpp for the Linux build, removing
// Windows-specific APIs (strcpy_s).
...
bool FunctionCallTip::getCursorFunction()
{
	...
	if (curValue.lastFunctionIdentifier > -1)
	{
		...
		if (!same)
		{
			...
			std::strncpy(_funcName.get(), funcToken.token, static_cast<size_t>(funcToken.length));
			_funcName.get()[funcToken.length] = '\0';
			res = loadFunction();
		}
		else
		{
