...
// Linux/Qt port of AutoCompletion.cpp
// This file replaces AutoCompletion.cpp for the Linux build, replacing
// Windows-specific APIs with POSIX/C++ equivalents:
// - WIN32_FIND_DATA / FindFirstFile / FindNextFile -> <filesystem>
// - GetModuleFileNameW / PathRemoveFileSpec -> NppParameters::getNppPath()
// - shlwapi.h path functions -> std::filesystem
// - _stricmp -> strcasecmp (via LinuxTypes.h)
...
// Linux path completion: uses forward slashes and std::filesystem
...
static bool isAllowedBeforePath(wchar_t c)
{
...
{
	// On Linux, look for paths starting with / or ~/
	// Search backwards for a path-starting character
	...
	for (size_t i = input.length(); i > 0; --i)
	{
		...
		if (input[idx] == L'/')
		{
			// Check if this could be start of an absolute path
			if (idx == 0 || isAllowedBeforePath(input[idx - 1]))
			{
				pathStart = idx;
				break;
			}
			// Keep scanning - this slash could be inside a path
			if (pathStart == std::wstring::npos)
				pathStart = idx; // tentative start, keep looking
		}
		else if (input[idx] == L'~' && idx + 1 < input.length() && input[idx + 1] == L'/')
		{
			if (idx == 0 || isAllowedBeforePath(input[idx - 1]))
			{
				pathStart = idx;
		...
		{
			// We found a slash earlier, and this non-slash char is before it
			// Check if the previous char is a path separator or path start
			if (input[idx] == L' ' || input[idx] == L'\t' || input[idx] == L'\'' || input[idx] == L'"' || input[idx] == L'(')
			{
				pathStart = idx + 1;
				break;
			}
	...
	rawPath_out = input.substr(pathStart);

	// Expand ~ to home directory
	if (!rawPath_out.empty() && rawPath_out[0] == L'~')
	{
		...
		if (home)
		{
			...
			for (const char* p = home; *p; ++p)
				homeW += static_cast<wchar_t>(*p);
			rawPath_out = homeW + rawPath_out.substr(1);
		}
	}

	return !rawPath_out.empty() && rawPath_out[0] == L'/';
}
...
void AutoCompletion::showPathCompletion()
{
	...
	// Get all files and directories in the path using std::filesystem
	std::wstring autoCompleteEntries;
	{
		std::wstring pathToMatchPlusSlash = addTrailingSlash(pathToMatch);

		// Convert wstring path to narrow string for filesystem
		std::string narrowPath;
		for (wchar_t wc : pathToMatchPlusSlash)
			narrowPath += static_cast<char>(wc);

		std::error_code ec;
		auto dirIter = std::filesystem::directory_iterator(narrowPath, ec);
		if (ec)
			return;
		...
		for (const auto& entry : dirIter)
		{
			...
			for (char c : entryName)
				autoCompleteEntries += static_cast<wchar_t>(c);
...
bool AutoCompletion::setLanguage(LangType language)
{
	...
	// On Linux, find autoCompletion XML files relative to the app directory
	const std::wstring& nppPath = NppParameters::getInstance().getNppPath();
	std::wstring path = nppPath + L"/autoCompletion/";
	path += getApiFileName();
	path += L".xml";
	...
	_funcCompletionActive = NppXml::loadFile(_pXmlFile, path.c_str());
...
{
	// Note: User-defined language autocompletion is not yet supported on Linux
	// because QtCore::Buffer doesn't have isUserDefineLangExt/getUserDefineLangName
